<template>
                    ...
                    <ul>
                        <li v-for="(item, index) in sub_dictionaries" :key="index" @click="changeDic(item)"
                            ...
                            <div class="folder-block">
                                ...
                                <el-button :icon="EditPen" circle @click.stop="showDialogRename(item)" v-show="item.show"
                ...
                <div style="margin-left: 35px;">
                    <ul>
                        <li v-for="(item, index) in formlist" :key="index" @click="editform(item)"
                            ...
                            <div class="folder-block">
                                <font-awesome-icon :icon="['fas', 'bars']"
                                    style="font-size: 30px; position: relative; top: -20px; left: 14px;" />
                                <div class="folder-name" style=" padding-left: 20%;">{{ item.name }}</div>
                                <el-button :icon="EditPen" circle @click.stop="editscreen(item)"
                                    style="float: right; position: absolute; top: 10px; right: 10px;" v-show="item.show" />
                                <div class="icon-close" v-show="item.show">
                                    <el-icon>
                        ...
                        <li v-for="(item, index) in screenlist" :key="index" @click="editscreen(item)"
                            ...
                            <div class="folder-block">
                                <font-awesome-icon :icon="['fas', 'object-group']"
                                    style="font-size: 30px; position: relative; top: -15px; left: 14px;" />
                                <div class="folder-name" style=" padding-left: 20%;">{{ item.name }}</div>
                                <el-button :icon="EditPen" circle @click.stop="showDialogRename(item)" v-show="item.show"
                                    style="float: right; position: absolute; top: 10px; right: 10px;" />
                                <div class="icon-close" v-show="item.show">
    ...
    <el-dialog v-model="initCreateDoc" title="创建新文件" width="30%" :before-close="handleInitDialog"
        style="border-radius: 20px;">
        <el-input v-model="createDocDialog.name" placeholder="请输入文件名称" clearable maxlength="20"
            style="font-size: 16px; margin-bottom: 15px" />
        <el-input v-model="createDocDialog.introduction" placeholder="请输入文件介绍" clearable maxlength="20"
            style="font-size: 16px; margin-bottom: 15px" />
        <template #footer>
            <span class="dialog-footer" style="margin-top: 500px;">
                <el-button @click="handleInitDialog">取消</el-button>
                <el-button type="primary" style="background-color:#1b9aee !important" @click="createDoc">
                    确定
                </el-button>
            </span>
        </template>
    </el-dialog>
    <el-dialog v-model="initCreateDic" title="创建新文件夹" width="30%" :before-close="handleDictDialog"
        style="border-radius: 20px;">
        <el-input v-model="dictDialog.name" placeholder="请输入文件夹名称" clearable maxlength="20"
            style="font-size: 16px; margin-bottom: 15px" />
        <template #footer>
            <span class="dialog-footer">
                <el-button @click="handleDictDialog">取消</el-button>
                <el-button type="primary" style="background-color:#1b9aee !important" @click="createDic">
                    确定
                </el-button>
            </span>
        </template>
    ...
    <el-dialog v-model="dialogVisible_rename" title="重命名" width="30%" :before-close="handleClose_rename"
        style="border-radius: 20px;">
        <el-input v-model="reName" placeholder="请输入新名称" clearable maxlength="20" style="font-size: 16px;" />
        <template #footer>
            <span class="dialog-footer">
                <el-button @click="handleClose_rename">取消</el-button>
                <el-button type="primary" style="background-color:#1b9aee !important" @click="renameDoc">
                    确定
                </el-button>
            </span>
        </template>
    </el-dialog>
    <el-dialog v-model="dialogVisible_renameDic" title="重命名" width="30%" :before-close="handleClose_renameDic"
        style="border-radius: 20px;">
        <el-input v-model="reNameDic" placeholder="请输入新名称" clearable maxlength="20" style="font-size: 16px;" />
        <template #footer>
            <span class="dialog-footer">
                <el-button @click="handleClose_renameDic">取消</el-button>
                <el-button type="primary" style="background-color:#1b9aee !important" @click="renameDic">
                    确定
                </el-button>
            </span>
        </template>
...
const handleClose_edit = (done: () => void) => {
    ElMessageBox.confirm('确定要取消编辑信息吗？')
        .then(() => {
            dialogVisible_edit.value = false
            reNameProject.value = null
            reIntroduction.value = null
        })
        .catch(() => {
            // catch error
        })
...
function workModule() {
    createDocDialog.module = '工作周报'
}
function projectModule() {
    createDocDialog.module = '项目计划'
}
function meetModule() {
    createDocDialog.module = '会议纪要'
}
function initModule() {
    createDocDialog.module = '空白文档'
}
function createDoc() {
    axios({
        // 接口网址：包含协议名，域名，端口和路由
        ...
        data: JSON.stringify({
            token: getUserInfo().token,
            project_id: route.params.pid,
            name: createDocDialog.name,
            introduction: createDocDialog.introduction,
            dictionary_id: selfTree.value.dictionary_id,
        }),
        // 成功请求回数据后，进入then，并用console.log打印结果
    }).then(res => {
        console.log(res)
        if (res.data.errno === 0) {
            store.commit('changeModule', createDocDialog.module)
            handleInitDialog()
            update()
...
function createDic() {
    axios({
        ...
        data: JSON.stringify({
            ...
            parent_id: selfTree.value.dictionary_id
        }),
        // 成功请求回数据后，进入then，并用console.log打印结果
    }).then(res => {
        console.log(res)
...
function edit_project() {
    axios({
        ...
        data: JSON.stringify({
            ...
            team_id: getUserInfo().currentTeam.id,
            project_id: route.params.pid,
            rename: reNameProject.value,
            reintroduction: reIntroduction.value
        }),
        // 成功请求回数据后，进入then，并用console.log打印结果
    }).then(res => {
        if (res.data.errno === 1007) {
