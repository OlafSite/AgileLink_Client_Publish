...
<template>
  <div class="design-container">
    ...
    <div class="main-body">
      ...
      <div class="main-form" v-loading="state.loading"  id="element-to-capture">
...
<script setup lang="ts">
  ...
  function getUserInfo() {
    return mystore.getters.getUserinfo
  }
  const page_id = mystore.state.Form.id
  function saveData() {
    axios({
      // 接口网址：包含协议名，域名，端口和路由
      ...
      data: JSON.stringify({
        ...
        page_id: page_id,
        type: 2,
        content:JSON.stringify(state.formData)
      }),
      // 成功请求回数据后，进入then，并用console.log打印结果
    }).then(res => {
      console.log('保存成功')
      router.push('/main/project/' + mystore.state.Doc.project_id);
    }).catch(err => {
      console.log(err)
    })
  }
  ...
  const getData = () => {
    axios({
      ...
      data: JSON.stringify({
        token: getUserInfo().token,
        page_id:mystore.state.Form.id

      }),
      // 成功请求回数据后，进入then，并用console.log打印结果
    }).then(res => {
       state.formData = JSON.parse(res.data.data.content)
    }).catch(err => {
      console.log(err)
    })
  }
