<template>
  ...
  <div class="container-screen" v-loading="state.loading">
    ...
    <button v-if="state.isEye" @click="state.isEye = false" class="Lcyclosebutton">关闭预览界面</button>
    <control-left :style="{ width: state.widthLeft }" ref="controlLeftEl" v-model:active="state.activeIndex"
      @update="controlEvent" />
    <!-- 顶不个工具条  -->
    <div class="main-box">
      <head-tools @click="headToolsClick">
        <div class="control-tools">
          <i class="icon-mouse" title="矩形选择工具" :class="{ active: state.activeTool === 'mouse' }"
            @click="state.activeTool = 'mouse'"></i>
          <i class="icon-hand" title="手拖动工具" :class="{ active: state.activeTool === 'hand' }"
            @click="state.activeTool = 'hand'"></i>
        </div>
      </head-tools>
      <div class="design-box" ref="designBoxEl" @scroll="designScroll">
        <a-ruler size="1920px" :scale="state.scale" :offset="state.offset" :scroll="state.scroll"
          :show-ruler="state.ruler" :show-line="state.showLine" />
        <a-ruler size="1080px" :scale="state.scale" :offset="state.offset" :scroll="state.scroll"
          :show-ruler="state.ruler" :show-line="state.showLine" direction="v" />
        <div @mousedown.left="onmousedown" class="design-canvas" :class="{ preview: state.isEye }" :style="canvasStyle"
          @contextmenu="contextmenu" id="element-to-capture">
          <draggable itemKey="id" :list="screenData.list" name="fade" class="drag" v-bind="{
            group: 'form',
            animation: 200,
            handle: '.none'
          }" @add="draggableAdd">
            <template #item="{ element, index }">
              <a-screen :data="element" @click.stop="itemClick(element, index)" :type="0" :scale="state.scale"
                :current="state.activeIndex.includes(index)" @contextmenu="screenContextmenu"
                @moveOrResize="moveOrResize" />
            </template>
          </draggable>
          <div class="draw-react" :style="state.rect" @contextmenu="drawReactContextmenu">
      ...
      <div class="design-footer">
        ...
        <div class="center">
          ...
          <div class="item slider">
            ...
            <el-slider show-stops :max="200" :min="state.autoScale - 5" size="small" :marks="marks"
              v-model="state.scale" />
          </div>
          <div class="item">
            <el-button type="primary" link @click="defaultScaleClick('auto')">自适应
            </el-button>
            <el-button type="primary" link @click="defaultScaleClick('100')">100%
            </el-button>
          </div>
        </div>
    ...
    <config-control :style="{ width: state.widthRight }" v-model:config="screenData.config" @open-drawer="openDrawer"
      @update="setLayerList" ref="configEl" />
...
import TableList from "@/views/design/dataList/components/list.vue";
...
const draggableAdd = (evt: any) => {
  ...
  // obj.position.left = parseInt(`${pageX}`)
  // obj.position.top = parseInt(`${pageY}`)
  obj.position.left = parseInt(`${pageX - state.offset[0] + state.scroll[0] + 180}`)
  obj.position.top = parseInt(`${pageY - state.offset[1] + state.scroll[1] + 40}`)
...
const headToolsClick = (type: string) => {
  switch (type) {
    ...
    case 'vue':
      ...
      break
    case 'html':
      vueFileEl.value.openScreen2(screenData.value)
      break
    case 'png':
      exportToImage()
      break
...
const page_id = store.state.Form.id
function saveData() {
  axios({
    // 接口网址：包含协议名，域名，端口和路由
    url: '#/api/v1/prototype_design/save_page_content',
    // 请求方式，默认为get，可以不写
    method: 'post',
    ...
    data: JSON.stringify({
      ...
      page_id: page_id,
      type: 1,
      content: JSON.stringify(screenData.value)
    }),
    // 成功请求回数据后，进入then，并用console.log打印结果
  }).then(res => {
    router.push('/main/project/' + store.state.Doc.project_id);
  }).catch(err => {
    console.log(err)
  })
}
...
const mygetData = () => {
  axios({
    // 接口网址：包含协议名，域名，端口和路由
    url: '#/api/v1/prototype_design/get_page_content',
    // 请求方式，默认为get，可以不写
    method: 'post',
    // 请求可以携带的参数，用对象来写，get方法对应params，其他方法对应data
    data: JSON.stringify({
      token: getUserInfo().token,
      page_id: store.state.Form.id

    }),
    // 成功请求回数据后，进入then，并用console.log打印结果
  }).then(res => {
    console.log('this is getDatatest')
    console.log(res.data.data.content)
    if (res.data.data.content.length === 0) {
      console.log('list里面没有东西')
    } else {
      screenData.value = JSON.parse(res.data.data.content)
    }
